function setupDropdown(dropdownButtonId, dropdownMenuId, searchInputId, resetButtonId = null) {
...
setupDropdown('options-menu', 'dropdown-menu', 'search-input', 'district-menu');
setupDropdown('district-menu', 'district-dropdown-menu', 'district-search-input');
...
async function populateStates() {
    ...
    for (let state of states) {
        ...
        div.className = 'cursor-pointer text-gray-700 hover:bg-gray-100 hover:text-gray-900 py-2 px-4';
        div.role = 'menuitem';
...
async function populateDistricts(state) {
    const response = await fetch(`http://localhost:3001/getDistricts?state=${state}`);
